<script>
  ...
  let countdown = writable(); // Reactive variable for the countdown
  ...
  async function startRecording() {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      ...
      mediaRecorder.addEventListener("stop", () => {
        const audioBlob = new Blob(audioChunks.slice(0, countdownDuration)); // Trim to duration
      ...
      isRecording = true;
      countdown.set(countdownDuration); // Set initial countdown value
      ...
      // Automatically stop recording after countdown duration
      recordingTimeout = setTimeout(stopRecording, countdownDuration * 1000);
    } else {
  ...
  function stopRecording() {
    if (audioRecorder) {
      ...
      isRecording = false;
      countdown.set(countdownDuration); // Reset countdown
    }
  }
...
<div class="container">
  ...
  {#if mode === "upload"}
    <div>
      ...
      />
      <p>File length max {countdownDuration} seconds...</p>
    </div>
  {/if}
  ...
  {#if mode === "record"}
    <div>
      ...
      {:else}
        <p>Can record up to 30 seconds...</p>
      {/if}
    </div>
  {/if}
